...
public class Last_GirlJung_Service extends Service {
    ...
    private double target[][] = SaJoData.girl_target_two;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        Cursor cursor = db.query("'" + Contact.MID_GIRL_WEIGHT_JUNG + "'", null, null, null, null, null, null);
    ...
    private void Traning_ManOne() {
        for (int all = 0; all < input_collection_girl.length; all++) {
            ...
            for (int k = 0; k < out_count; k++) {
                double differential = (n * (target[all][1] - out[all][k]) * (1 - out[all][k]) * out[all][k]); //target 가운데만 되게
        ...
        protected void onPostExecute(Integer o) {
            ...
            for (int i = 0; i < out.length; i++) {
                for (int j = 0; j < out[i].length; j++) {
                    sumHangle += GetHangle.Girl_JungSung_String[(int) GetNearValue.getNearJUNG(out[i][j])];
                }
            }
            Log.e("마지막 여자 중성", sumHangle);
            SQLiteDatabase db = dbManager.getWritableDatabase();
            ...
            db.insert("'" + Contact.LAST_GIRL_WEIGHT_JUNG + "'", null, values);
            db.close();
