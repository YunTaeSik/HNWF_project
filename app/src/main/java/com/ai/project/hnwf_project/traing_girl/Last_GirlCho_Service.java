...
public class Last_GirlCho_Service extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        Cursor cursor = db.query("'" + Contact.LAST_GIRL_WEIGHT_CHO + "'", null, null, null, null, null, null);
        ...
        protected void onPostExecute(Integer o) {
            ...
            for (int i = 0; i < out.length; i++) {
                for (int j = 0; j < out[i].length; j++) {
                    sumHangle += GetHangle.Girl_ChoSung_String[(int) GetNearValue.getNearCHO(out[i][j])];
                }
            }
            Log.e("마지막 여자 초성", sumHangle);
            ...
            db.insert("'" + Contact.LAST_GIRL_WEIGHT_CHO + "'", null, values);
            db.close();
